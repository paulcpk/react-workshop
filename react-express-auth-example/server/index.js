...
const getUserByEmail = ({ email }) => USER_DATA.find((user) => user.email === email)

const getUserByCredentials = ({ email, password }) =>
  USER_DATA.find((user) => user.email === email && password === user.password)
...
app.get('/api/profile', (req, res) => {
  ...
  if (req.user) {
    const user = getUserByEmail({ email: req.user })
    console.log('user', user)
    res.json({ data: user })
  } else {
    res.status(401).send('Unauthorized')
